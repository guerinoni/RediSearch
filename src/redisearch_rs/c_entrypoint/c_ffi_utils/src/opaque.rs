...
/// An trait for using a sized type in an FFI context as an opaque sized type,
    ...
    ($ty:ty, $opaque_ty:ident) => {
        mod __opaque {
            ...
            impl $crate::opaque::IntoOpaque for $ty {
                ...
                fn into_opaque(self) -> Self::Opaque {
                    // Safety: `Self::Opaque` is defined as a `MaybeUninit` slice of
                    // bytes with the same size and alignment as `Self`, so any valid
                    // `Self` has a bit pattern which is a valid `Self::Opaque`.
